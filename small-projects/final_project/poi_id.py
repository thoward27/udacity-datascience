...
GRID = {
    "GRID:DecisionTree": GridSearchCV(
        DecisionTreeClassifier(random_state=SEED),
        {
            ...
            'max_features': [None, 2, 3, 4, 5, "auto", "sqrt", "log2"],
...
def _get_dataset(data_dir="final_project_dataset.pkl"):
    """ Loading / cleaning the data """
    data = d2u.pickle_load(data_dir)
    # remove outliers
    del data['TOTAL']
    return data
...
def _test_classifiers(CLASSIFIERS, x_train, x_test, y_train, y_test):
    ...
    for group in CLASSIFIERS:
        for name, clf in group.items():
            ...
            results[name]['pred'] = clf.predict(x_test)
...
if __name__ == "__main__":
    ...
    results = _test_classifiers(CLFS, x_train, x_test, y_train, y_test)

    print(""" analyzing results """)
    dataframe = pd.DataFrame.from_dict(results, orient="index")

    print(""" exporting results """)
    # remove fields not for analysis
    dataframe.drop(['pred'], axis=1, inplace=True)
    with shelve.open('results', flag="c", writeback=True) as shelf:
        shelf['0'] = dataframe
        dataframe.to_csv('results.csv', index_label="name")
